    ...
    def optimize_fast(constraints_effective: pd.DataFrame, supplements: pd.DataFrame):
        ...
        # 0) Fehlende benötigte Nährstoff-Spalten in der DB (harte Infeasibility)
        ...
        # 1) Grundnahrung allein überschreitet Max
        ...
        if supp_clean.shape[0] == 0:
            return "Infeasible", None, None, {"Hinweis": "Nach Cleaning sind keine Supplements mit Nährstoffgehalt übrig."}, {"reason": "empty_supp_clean"}

        # ---------- Diagnose-Setup ----------
        MIN_KG = 1.0 / 1000.0   # = 1g Mindestmenge wenn gewählt
        BIG_M_KG = 10.0         # max kg/Tag pro Supplement (wenn gewählt)

        def _diagnose():
            """
            Heuristische Diagnose:
            - Min > grob maximal erreichbar unter BIG_M_KG
            - Min>0 aber keine positive Quelle (alle coeff <= 0)
            - Grundnahrung > Max (Max-Base < 0)
            - zeigt Top-Quellen + 1g-Effekt
            """
            diag_rows = []
            ...
            for n in nutrient_cols:
                ...
                pos_sum = float(np.maximum(coeff, 0.0).sum())
                ...
                if min_needed > 0 and (not has_positive_source):
                    reason_flags.append("Min>0 aber keine positive Quelle (alle coeff ≤ 0)")
                if min_needed > max_possible + 1e-12:
                    reason_flags.append("Min größer als maximal erreichbar (BIG_M Limit)")
                if pd.notna(max_allowed) and max_allowed < 0:
                    reason_flags.append("Grundnahrung überschreitet Max (Max-Base < 0)")

                diag_rows.append({
                    "Nährstoff": n,
                    "Min_nach_Grundnahrung": min_needed,
                    "Max_nach_Grundnahrung": (max_allowed if pd.notna(max_allowed) else np.nan),
                    "Max_erreichbar_grob": max_possible,
                    "Hat_positive_Quelle": has_positive_source,
                    "Flags": " | ".join(reason_flags),
                    "Top_Quellen_coeff_pro_kg": top_sources_dict,
                    "1g_Effekt_TopQuellen": one_g_effect_dict,
                })
            ...
            likely = diag_df[
                ...
                | (diag_df["Min_nach_Grundnahrung"] > diag_df["Max_erreichbar_grob"] + 1e-12)
            ].copy()

            return {
                "diag_df": diag_df,
                "likely_problem_nutrients": likely.sort_values(
                    by=["Min_nach_Grundnahrung"], ascending=False
        ...
        # Wenn Base>Max irgendwo: sofort Diagnose zurück (sehr häufige Ursache)
        if infeasible_msg:
            dbg = {
                "nutrient_cols": nutrient_cols,
            ...
            return "Infeasible", None, None, {"Grundnahrung_ueber_Max": infeasible_msg}, dbg
        ...
        debug = {
            ...
            "min_req": min_req,
            "max_req": max_req,
            "diagnose": _diagnose(),  # auch bei Erfolg optional
        }
    ...
    with st.expander("Optimierung (aufklappen)", expanded=True):
        if st.button("🚀 Optimierung starten", disabled=not can_run):
            ...
            if status != "Optimal" or solution is None or debug is None:
                ...
                # (A) Frühdiagnose (z.B. Grundnahrung>Max oder Missing Columns)
                if infeasible_msg:
                    st.info("Erste Diagnose / Hinweis:")
                    st.json(infeasible_msg)

                # (B) Detaildiagnose aus debug["diagnose"]
                diag = None
                if isinstance(debug, dict):
                    diag = debug.get("diagnose", None)

                if diag is not None:
                    with st.expander("🧩 Diagnose-Details (warum keine Lösung?)", expanded=True):
                        st.caption(
                            "Heuristik: (1) Min > maximal erreichbar (BIG_M), (2) keine positive Quelle, "
                            "(3) Grundnahrung > Max, plus Top-Quellen & 1g-Effekt."
                        )

                        likely = diag.get("likely_problem_nutrients", None)
                        if isinstance(likely, pd.DataFrame) and not likely.empty:
                            st.markdown("#### ⚠️ Verdächtige Nährstoffe (wahrscheinlich Ursache)")
                            show = likely[[
                                "Nährstoff",
                                "Min_nach_Grundnahrung",
                                "Max_nach_Grundnahrung",
                                "Max_erreichbar_grob",
                                "Hat_positive_Quelle",
                                "Flags"
                            ]].copy()
                            show = show.sort_values(by=["Flags", "Min_nach_Grundnahrung"], ascending=[False, False]).reset_index(drop=True)
                            st.dataframe(show, use_container_width=True, hide_index=True)

                            st.markdown("#### 🔍 Top-Quellen & 1g-Effekt (für die schlimmsten 5)")
                            st.json(likely.head(5).to_dict(orient="records"))
                        else:
                            st.warning(
                                "Keine klaren Flags gefunden. Dann ist es oft ein Konflikt zwischen Min/Max über mehrere Nährstoffe "
                                "oder ein Einheitenproblem (mg vs g)."
                            )

                        full = diag.get("diag_df", None)
                        with st.expander("Alle Nährstoffe (Full Table)", expanded=False):
                    ...
                    st.info("Keine Diagnose verfügbar (debug enthält keine diagnose).")
